	...
	if (type == 0) {
		...
		if (ce) {
			...
			if (EXPECTED(yaf_is_name_suffix())) {
				yaf_trigger_error(YAF_ERR_AUTOLOAD_FAILED,
					"Could not find class %s in controller script %s",ZSTR_VAL(controller), directory);
			} else {
				yaf_trigger_error(YAF_ERR_AUTOLOAD_FAILED,
		...
		if (Z_TYPE_P(action_map) != IS_ARRAY) {
			yaf_trigger_error(YAF_ERR_NOTFOUND_ACTION,
					"There is no method %s in %s", ZSTR_VAL(action), ZSTR_VAL(Z_OBJCE_P(controller)->name));
		} else if ((pzval = zend_hash_find(Z_ARRVAL_P(action_map), action)) == NULL) {
			yaf_trigger_error(YAF_ERR_NOTFOUND_ACTION, "There is no method %s in %s::$%s", ZSTR_VAL(action),
...
static zend_class_entry *yaf_dispatcher_get_controller(zend_string *app_dir, yaf_request_object *request, int def_module) /* {{{ */ {
	...
	if (def_module) {
		...
		lc_name =strpprintf(0, "app\\%s\\%s", YAF_CONTROLLER_DIRECTORY_NAME, ZSTR_VAL(controller));
	} else {
		if (UNEXPECTED(ZSTR_LEN(app_dir) + ZSTR_LEN(module) + sizeof(YAF_CONTROLLER_DIRECTORY_NAME) > MAXPATHLEN)) {
			return yaf_dispatcher_get_errors_hub(0, app_dir, module, controller);
		}
		directory_len = yaf_compose_3_pathes(directory, app_dir, module, ZEND_STRL(YAF_CONTROLLER_DIRECTORY_NAME));
		lc_name = strpprintf(0, "app\\%s\\%s\\%s", ZSTR_VAL(module), YAF_CONTROLLER_DIRECTORY_NAME, ZSTR_VAL(controller));
	}
	lc_name = zend_string_tolower(lc_name);
		...
		if ((ce = yaf_dispatcher_get_controller(app->directory, request, is_def_module))) {
			...
			if (EXPECTED(yaf_view_get_tpl_dir(&dispatcher->view, &dispatcher->request) == NULL)) {
				...
				if (is_def_module) {
					...
					view_dir = strpprintf(0, "%s%c%s%c%s", ZSTR_VAL(app->directory),
							DEFAULT_SLASH, ZSTR_VAL(request->module), DEFAULT_SLASH, "views");
				}
			...
			do { /* Fetch the action method, if it doesn't exist , fall to $action_map property */
				...
				func_len = ZSTR_LEN(request->action);
...
ZEND_HOT yaf_response_t *yaf_dispatcher_dispatch(yaf_dispatcher_object *dispatcher) /* {{{ */ {
	...
	if (EXPECTED(yaf_request_is_dispatched(request))) {
		...
		// 	yaf_response_clear_body(Z_YAFRESPONSEOBJ(dispatcher->response), NULL);
		// }
		//php_printf("response is :%s",dispatcher->response->getBody());
